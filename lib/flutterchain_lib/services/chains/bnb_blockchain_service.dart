...
class BNBBlockChainService implements BlockChainService {
  ...
  Future<EVMTransactionInfo> getTransactionInfo(String adress) async {
    final res = await bnbRpcClient.getTransactionInfo(adress);
    return res;
  }
  ...
  Future<String> getWalletBalance(TransferRequest transferRequest) {
    return bnbRpcClient.getAccountBalance(transferRequest.accountID!);
  }
  ...
  Future<void> setBlockchainNetworkEnvironment({required String newUrl}) async {
    bnbRpcClient.networkClient.setUrl(newUrl);
  }
