...
export function najPublicKeyStrToUncompressedHexPoint(publicMPCKey) {
  const res = '04' + Buffer.from(bs58.decode(publicMPCKey.split(':')[1])).toString('hex');
  return res;
}
...
export async function uncompressedHexPointToBtcAddress(publicKeyHex, network) {
  ...
  const network_byte = network === 'mainnet' ? 0x00 : 0x6f;
